...
type RecursionProps = (id: string, array: TodoType[]) => TodoType[];

type SearchProps = (id: string, array: TodoType[]) => TodoType | null;

type CompleteTogglerProps = (array: TodoType[], state: boolean) => TodoType[];
...
export const recursionFilter: RecursionProps = (id, array) => {
  return array &&  array.reduce((arr: TodoType[], item) => {
    if (item.id !== id) {
  ...
  return array.reduce((arr: TodoType[], item) => {
    if (item.id !== id) {
      ...
      arr.push({
        ...
        subTasks: subTaskCompleteToggler(item.subTasks, !item.isCompleted),
      });
    }
...
export const subTaskCompleteToggler: CompleteTogglerProps = (array, state) => {
  return array.reduce((arr: TodoType[], item) => {
    arr.push({
      ...item,
      isCompleted: state,
      subTasks: subTaskCompleteToggler(item.subTasks, state),
    });
